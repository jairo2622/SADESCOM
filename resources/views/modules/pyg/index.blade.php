...
@section('contenido')
    <h2 class="text-center mt-3 mb-4">GANANCIAS</h2>
    ...
    <div class="container">
        <div class="row">
            <div class="col-6">
                ...
                <a href="{{ route('perdidas') }}" class="btn btn-danger d-inline-flex align-items-center gap-1"><svg
                        xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        class="bi bi-eye-fill" viewBox="0 0 16 16">
                        <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0" />
                        <path
        ...
        @php
            ...
            $ventasAgrupadas = $items->groupBy(function ($venta) {
                return Carbon::parse($venta->created_at)->format('Y-m');
            });
        @endphp

        @foreach ($ventasAgrupadas as $periodo => $ventas)
            @php
            ...
            <h2 class="mt-4 month-title" data-periodo="{{ strtolower($fechaTitulo) }}">
                ...
                <table class="table table-striped table-bordered shadow text-center p-3 tabla_pyg table_apartados ">
                            ...
                            <tr>
                                <td>
                                    <table class="table table-sm table-bordered mb-0 table_apartados ">
                                        ...
                                        <tbody>
                                            @php
                                                $productos = is_array($item->productos)
                                                    ? $item->productos
                                                    : json_decode($item->productos, true) ?? [];
                                            @endphp
                                            ...
                                            @foreach ($productos as $producto)
                                                <tr>
                                                    ...
                                                    <td>${{ number_format($producto['total'] ?? 0, 0, ',', '.') }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
